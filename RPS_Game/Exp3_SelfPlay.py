...
if __name__ == '__main__':
        ...
        # Save regrets and KL_DIVERGENCE to CSV files
        regrets_df = pd.DataFrame(regrets)
        tv_df = pd.DataFrame(kl_divergences)

        regrets_df.to_csv(os.path.join(output_dir, f'exp3_selfplay_regrets.csv'), index=False)
        tv_df.to_csv(os.path.join(output_dir, f'exp3_selfplay_kl_divergences.csv'), index=False)
